...
class SerialGUI(QWidget): // creating the main GUI window
    def __init__(self):  // self refers to our gui window
        ...
        title = QLabel("Sound Sensor Monitor") // title 
...
// Information Labels
        ...
        for lbl in [ // styling the labels and applying parameters to all of them
            self.level_label,
...
// Progress bar itself
	    ...
	    // Loud and Quiet indicators inside the Progress bar
...
def read_serial(self):
        if ser.in_waiting > 0: // ser is python's connection to arduino (com17, 115200....) and it checks whether the serial data is available
            line = ser.readline().decode(errors="ignore").strip() //strip removes \n, decode bytes into a string

            if line:
                parts = line.split(",") // separates the received fields
...
// limits the bar value by keeping it betwen 0 and 10
                        if level_value < 0: 
                            level_value = 0
                        if level_value > 10:
                            level_value = 10

                        self.level_bar.setValue(level_value)// updates progress bar
                        if level_value >= threshold_value:
                            self.level_bar.setStyleSheet("""
                                QProgressBar {
                                    border: 2px solid gray; // color of progress bar
                                    border-radius: 5px;
                                    text-align: center;
                                }
                                QProgressBar::chunk {
                    ...
                    if "Above=1" in above: // it is updating LOUD/QUIET status
